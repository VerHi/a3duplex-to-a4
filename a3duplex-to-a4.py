...
def main():
    parser = argparse.ArgumentParser(description='A3 duplex scanned PDF to A4 PDF.')
    ...
    parser.add_argument('-r', '--rotate', nargs='?', type=int, default=-1, choices=range(0, 3), help='Rotate PDF by 90(0)|180(1)|270(2) degrees clockwise.')
    parser.add_argument('-q', '--quality', nargs='?', type=int, default=90, help='JPEG compression quality.')
    parser.add_argument('--dpi', nargs='?', type=int, default=300, help='Source DPI.')
    parser.add_argument('--raw', nargs='?', type=str, help='Folder to store cropped image files in. Disables PDF creation.')
    ...
    for file in args.input:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(['pdfimages', '-all', str(filePath), str(Path(tmp, 'source'))])
            if args.corrupted:
                for f in Path(tmp).glob('source-*'):
                    logging.info(f'Reprocessing image {f}.')
                    subprocess.run(['magick', 'mogrify', '-strip', str(Path(tmp, f))], stderr=subprocess.DEVNULL)
            ...
            if args.raw:
                ...
                print('Creating PDF.')
                createpdf([str(file) for file in Path(tmp).glob('crop-*.*')], output, args.dpi)
...
def cropimages(folder, quality, rotate):
    ...
    for i in range(realPages):
        logging.info(f'Creating page {i} from image {mappedPages[i]//2:03d}, quality {quality:03d}.')
        ...
        cv2.imwrite(str(Path(folder, f'crop-{i:03d}'+file.suffix)), crop, encode_param)
